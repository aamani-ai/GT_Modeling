...
- **Coefficient is a literature placeholder** (`0.004/°F`, GER-3620-style). The load-temp paper has now landed (Saturday & Isaiah 2018, register §9) and surfaces a **~17× higher** ambient creep sensitivity than our current value (6.85%/°F on LM2500+ aero-derivative vs our 0.4%/°F). Heavy-duty F-class vs aero-derivative likely explains some of the gap — **sensitivity sweep needed before adoption** (register §3.7). The *mechanism* is committed; the *number* is revisable.
...
- **Coefficients** (`P_CREEP_*`, `TRIP_MAINTENANCE_FACTOR`) are Bucket-B placeholders — Saturday & Isaiah 2018 (the load-temp paper, register §9) is creep-only and doesn't directly address `TRIP_MAINTENANCE_FACTOR`; that figure remains a GER-3620 convention pending OEM-specific evidence. Full record: [`docs/decisions/007-creep-wiring-and-trip-wear.md`](../../../docs/decisions/007-creep-wiring-and-trip-wear.md).
