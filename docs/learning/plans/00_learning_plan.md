...
This learning area is for building up gas plant modeling knowledge from the basics before going deep into degradation factors. The target reader is new to gas turbine performance modeling, LTSA terms, plant-type differences, and daily dispatch feedback. The docs should explain the ideas slowly enough to learn from, while using the InfraSure Athens-type GE 7FA combined-cycle case as the first worked example rather than the universal default.

The first writing phase should cover core vocabulary and plant anatomy:

- Combined-cycle plant anatomy and acronyms
- Gas plant type map and plant-type applicability
- Capacity
- Heat rate
- EOH and starts
...
The current learning docs use Athens as the first worked CCGT example. Future edits should separate universal gas-plant concepts from plant-type-specific assumptions.
...
- Label Athens values as worked-example assumptions, not universal gas-plant constants.
- Add plant-type notes when equipment, contract, dispatch, or degradation assumptions differ by plant type.
...
- Call out where a term is universal vs plant-type-specific.
...
| Done | Degradation applicability notes | Adds plant-type applicability warnings so GT-only and HRSG/ST-only factors are not treated as universal gas-plant assumptions. |
...
| :--- | :--- | :--- | :--- |
| 0 | `basics/00_gas_plant_type_map.md` | What kinds of gas plants can this learning system cover? | Simple-cycle GT, combined-cycle GT, frame vs aero, CHP/cogeneration, optional reciprocating-engine scope, applicability matrix. |
| 0.1 | `basics/00_combined_cycle_plant_anatomy.md` | What are GT, ST, HRSG, BOP, LTSA, and the CCGT plant acronyms? | Combined-cycle flow, equipment map, 2x1/1x1 meaning, BOP, emissions systems, inspection acronyms, units glossary. |
...
| 8 | `basics/08_ltsa_and_service_contracts.md` | What is the service contract and how does it convert operation into cashflow and risk allocation? | LTSA/CSA basics, fees, reserves, overages, inspections, coverage, exclusions, guarantees, and open diligence questions. |
| 9 | `basics/09_operating_partitions_and_model_signals.md` | How should one operating trace be sliced into useful model signals? | Load mode, start type, dispatch profile, season, stress accumulation, price regime, ASCII load profiles, and Step 2 handoff signals. |

### Basics Guide Details

#### 0. Gas Plant Type Map

Core teaching goal: make plant type explicit before teaching CCGT-specific anatomy or Athens-specific assumptions.

Must explain:

- Universal gas-plant concepts vs plant-type-specific details.
- Simple-cycle GTs.
- Combined-cycle GTs.
- Frame vs aeroderivative GTs.
- CHP / cogeneration plants.
- Optional future scope for gas reciprocating engines.
- Why Athens is a worked CCGT example, not a universal gas-plant rule.
- Which degradation factors apply to which plant types.
- Why Pmin/Pmax, weather effects, offer curves, and historical-vs-forecast inputs may need a later market/operations layer.

Use this basic framing:

```text
universal gas-plant concept
        |
        v
...
#### 0.1. Combined-Cycle Plant Anatomy
...
- `docs/learning/plans/00_learning_plan.md` exists.
- `docs/learning/basics/` exists.
- `docs/learning/plant_types/` exists.
- `docs/learning/examples/` exists.
- `docs/learning/market_and_operations/` exists.
...
The next detailed writing pass should be chosen based on the modeling need:

- create `docs/step_2_execution_blueprint/00_index.md` if the next priority is starting the execution-blueprint folder;
- create `docs/step_2_execution_blueprint/01_time_resolution_and_frequency.md` if the next priority is resolving hourly, daily, monthly, and annual model frequencies;
- write `market_and_operations/05_dispatch_constraints.md` if ramp limits, minimum up/down time, start time, and commitment feasibility are the next priority;
- continue deepening degradation guides if the current Athens CCGT workflow remains the focus.
